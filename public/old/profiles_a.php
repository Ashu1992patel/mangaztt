	...
	if(isset($_SESSION["admin"]))
	{
		...
		{	extract($_POST);
			...
			date_modify($date,"+6 month");
				$set="UPDATE activate SET active='yes', plan='$plan', deactivated_at='".date_format($date,"Y-m-d")."' WHERE id=".$temp_id;
				if($conn->query($set)){
...
if($plan=="Diamond"){
			...
			if(!empty($_GET["prflid"])){
				//extract($_POST);
				echo "<table class='results'>";
				$pic_sql="SELECT pic1, pic2, pic3 FROM images WHERE id=".$_GET["prflid"];
				$detail_sql="SELECT * FROM profiles WHERE id=".$_GET["prflid"];
				$pic= mysqli_fetch_assoc(mysqli_query($conn, $pic_sql));
				$detail= mysqli_fetch_assoc(mysqli_query($conn, $detail_sql));
				echo "<tr><div style='color: purple; padding: 0 5px;'><strong><u>".$detail["name"]."</u> (".$detail["id"].")</strong></div></tr>";
				echo "<tr valign='top'>";
				...
				echo "<tr valign='top' >";
				echo "<table class='results' >";
				echo "<br><tr valign='top'><td class='details_heading'>Personal Details:<br></td></tr>";
			...
			{			
			$sql="SELECT * FROM profiles ORDER BY id DESC LIMIT 0, 300";
			$result=mysqli_query($conn,$sql);
			$msg="Most Recent Profiles...";
			if(mysqli_num_rows($result)==0)
			{	$msg="No profiles in the database...";
				
			}
			echo "<table class='results'>";
			echo "<tr><div style='color: purple; padding: 0 5px;'>".$msg."</div></tr>";
			while($row= $result->fetch_assoc())
			{
				...
				{	
					...
					$activate_sql="SELECT * FROM activate WHERE id=".$row["id"];
					...
					echo "<tr valign='top'><td class='details'><strong>Age: </strong>".$age->format("%y")." year &nbsp;"."<strong>HEIGHT: </strong>".$row["height"]."</td></tr>";
					echo "<tr valign='top'><td class='details'><strong>Caste: </strong>".$row["caste"]." &nbsp;"."<strong>Manglik: </strong>".$row["manglik"]."</td></tr>";
					echo "<tr valign='top'><td class='details'><strong>Email: </strong>".$row["email"]."</td></tr>";
					echo "<tr valign='top'><td class='details'><strong>Wrd: </strong>".$row["password"]."</td></tr>";
					
					echo "<tr valign='top'><td class='details'><strong>Marital Status: </strong>".$row["status"]."</td></tr>";
					...
					echo "<tr valign='top'><td class='details'><strong>Income: </strong>".$row["salary"]."</td></tr>";
					echo "<tr valign='top'><td class='details'><strong>Profile SignUp:</strong>".$row["date_created"]."</td></tr>";
					echo "<tr valign='top'><td class='details'><strong>Active Plan: </strong>".$activate["plan"]."</td></tr>";
					...
					echo "<form name='set_active' action='' onsubmit=confirm('Confirm this operation?') method='post'>";
					echo "<input type='hidden' name='temp_id' value='".$row["id"]."'>";
					
					if($check["active"]=="no"){
						echo "<tr valign='bottom'><td class='details' align='left'><select name='plan'><option value='Gold'>Gold</option>";
						echo "&nbsp;&nbsp;<option value='Super Gold'>Suer Gold</option>";
						echo "&nbsp;&nbsp;<option value='Diamond'>Diamond</option></select>&nbsp;&nbsp;<input type='submit' name='activate' value='Activate'></td>";
					}
					else
						echo "<tr valign='bottom'><td class='details' align='left'><font color='green'><strong>Active now</strong></font>&nbsp;&nbsp;<input type='submit' name='deactivate' value='Deasctivate'></td>";
					echo "</form>";
					...
					echo "<input type='hidden' name='temp_id' value='".$row["id"]."'>";
					echo "<td class='details' align='right'><a class='gpd' href='?prflid=" . $row["id"]. "'>View Full Profile</a></td></tr>";
