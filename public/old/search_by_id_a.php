...
<?php
	...
	if(isset($_SESSION["admin"]))
	{
		...
		{	extract($_POST);
			...
			if(isset($activate)){
				$set="UPDATE activate SET ctive='yes' , plan='$plan', activated_at='now()' WHERE id=".$temp_id;
				if($conn->query($set)){
					echo "<font color='green'>Activated successfully!</font>";
				}
			}elseif(isset($deactivate)){
				$set="UPDATE activate SET ctive='no' ,plan='free', detivated_at='now()' WHERE id=".$temp_id;
				if($conn->query($set)){
			...
			{	
				...
				$sql="SELECT * FROM profiles WHERE id='$id' or email='$id' or name='$id' or city='$id' or mob='$id' or mob2='$id' ";
			...
			{	
				...
				//if($check["active"]==="yes")
					...
					echo "<tr valign='top'><td class='details'><strong>Name: </strong>".$row["name"]." (".$row["id"].") <strong>Pass:</strong>".$row["password"]." <td class='details' align='right'><strong>D.O.B.: </strong>".$row["dob"]."</td></tr>";
					...
					if($check["active"]=="no"){
						echo "<tr valign='bottom'><td class='details' align='left'>
									<select name='plan'>
									 <option value='Gold'>Gold</option>
									 <option value='SuperGold'>SuperGold</option>
						 			 <option value='Diamond'>Diamond</option>
									</select>
						<input style='width:200px; align=center;' type='submit' name='activate' value='Activate'>";
					...
					echo "<form name='getdetails' action='' method='post'>";
					echo "<input type='hidden' name='temp_id' value='".$row["id"]."'>";
					echo "<tr valign='bottom'><td></td><td class='details' align='right'><input type='submit' name='show' value='Get Details'></td></tr>";
