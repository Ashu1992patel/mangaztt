	...
	if(isset($_SESSION["me"]))
	{
		...
		{	//extract($_POST);
			if(!empty($_GET["prflid"])){
				...
				echo "<tr valign='top'><td class='details'><strong>State & City of Residence: </strong>".$detail["city"].", ".$detail["state"]."</td><td class='details'><br><strong>Address: </strong>".$detail["address"]."</td></tr>";
				echo "<tr><td></td></tr>";
				...
				echo "<tr valign='top'><td class='details'><strong>Email ID: </strong>".$detail["email"]."</td></tr>";
					...
					echo "<tr> <td><center> <a href='payment.php'><div style='color: red;'> <br><u>Confirm payment to get contact details of this person...</u></div></a> </center></td></tr>";
			...
			{			
			$sql="SELECT * FROM profiles WHERE caste='".$_SESSION["me"]["caste"]."' and gender!='".$_SESSION["me"]["gender"]."' and manglik='".$_SESSION["me"]["manglik"]."' and status='".$_SESSION["me"]["status"]."' ORDER BY ID DESC LIMIT 0,1000 ";
			$result=mysqli_query($conn,$sql);
			$msg="Matching profiles for you...";
			if(mysqli_num_rows($result)==0)
			{	$msg="No profiles with your caste are currently available. Have a look on profiles with your religion...";
				$sql="SELECT * FROM profiles WHERE religion='".$_SESSION["me"]["religion"]."' and gender!='".$_SESSION["me"]["gender"]."' and manglik='".$_SESSION["me"]["manglik"]."' and status='".$_SESSION["me"]["status"]."'ORDER BY ID DESC LIMIT 0, 1000";
				$result=mysqli_query($conn,$sql);
				if(mysqli_num_rows($result)==0)
				{	$msg="No profiles with your caste or religion are currently available. Have a look on all other profiles...";
					$sql="SELECT * FROM profiles WHERE gender!='".$_SESSION["me"]["gender"]."' ORDER BY ID DESC";
					$result=mysqli_query($conn,$sql);
					if(mysqli_num_rows($result)==0)
						$msg="Perhaps, there is an error. We are trying to fix this...:-(";
				}
			}
			echo "<table class='results'>";
			...
			while($row= $result->fetch_assoc())
			{
				...
				{	
					...
					echo "<tr valign='bottom'><td></td><td class='details' align='right'><a class='gpd' href='?prflid=" . $row["id"]. "'>View Full Profile</a></td></tr>";
