...
class ProfileController extends Controller
{
    ...
    function index()
    {
        ...
        $users = DB::table('profiles')->orderBy('id', 'desc')->paginate(200);
    ...
    function admin_s_user()
    {
        ...
        $users = DB::table('profiles')->where('id', $user_id)->orWhere('name', 'like', "$user_id%")->orWhere('email', 'like', "$user_id%")->orWhere('contact', 'like', "$user_id%")->orWhere('name', 'like', "$user_id%")->orWhere('mob2', 'like', "$user_id%")->orWhere('landline', 'like', "$user_id%")->paginate(5);
    ...
    function user_list()
    {
        if (request('type') == 'active') {
            $users = DB::table('profiles')->where(['is_active' => 1])->orderBy('id', 'desc')->paginate(200);
            return view('user.admin_candidate_list')->with(['users' => $users]);
        } else if (request('type') == 'inactive') {
            $users = DB::table('profiles')->where(['is_active' => 0])->orderBy('id', 'desc')->paginate(200);
            return view('user.admin_candidate_list')->with(['users' => $users]);
    ...
    function all_user()
    {
        if (request('type') == 'paid') {
            ...
            $users = DB::table('profiles')->whereIn('id', $ab)->orderBy('id', 'desc')->paginate(200);
            return view('user.admin_candidate_list')->with(['users' => $users]);
