...
//        $search_users = DB::select("SELECT * FROM `profiles` WHERE gender = '$gender' or (age >= '$age1' or age <= '$age2') or religion = '$religion'");
    ...
    public function candidate_list(Request $request)
    {
        if (!isset($_SESSION['user_master'])) {
            ...
            if ($religion == 'Any')
                $users = DB::table('profiles')->select(DB::raw('*'))->whereBetween('age', ["$age1", "$age2"])->where(['gender' => $gender, 'is_verified' => 1])->orderBy('age', 'DESC')->paginate(5);
            else
                $users = DB::table('profiles')->select(DB::raw('*'))->whereBetween('age', ["$age1", "$age2"])->where(['religion' => $religion, 'gender' => $gender, 'is_verified' => 1])->orderBy('age', 'DESC')->paginate(5);
...
//        else
            ...
            $_SESSION['age1'] = $urs->gender == 'male' ? $urs->age - 4 : $urs->age;
            $_SESSION['age2'] = $urs->gender == 'male' ? $urs->age : $urs->age + 4;
...
//            dd($_SESSION);
            ...
            if ($religion == 'Any')
                ...
                $users = Profiles::whereBetween('age', ["$age1", "$age2"])->where(['religion' => $religion, 'gender' => $urs->gender == 'male' ? 'female' : 'male', 'caste' => $urs->caste, 'status' => $urs->status, 'is_verified' => 1])->orderBy('id', 'desc')->paginate(5);
...
//            return view('web.candidate_list')->with(['users' => $users, 'user_count' => 0]);
    ...
    public function get_search_advance(Request $request)
    {
        ...
        if (isset($_SESSION['user_master'])) {
            ...
            else if ($family_status == "Upper Middle")
                $f_status = " f_status='Upper Middle'";
            else if ($family_status == "Middle")
                $f_status = " f_status='Middle'";
            else
                $f_status = " 0 = 0";
            ...
            else if ($manglik == "Non vegetarian")
            ...
            $sql = "SELECT * FROM `profiles` WHERE is_verified = 1 and (gender='" . $gender . "') and (" . $status . " 0) and (" . $lang . " 0) and (" . $rel . " 0) and (" . $cast . " 0) and (" . $edu . " 0) and (" . $occu . " 0) and (" . $stt . " 0) and (" . $mang . ") and (" . $horoscope . ") and (" . $phy . ") and (" . $familyin . " 0) and (" . $anual . " 0) and (" . $diet . ") and (" . $f_status . ") and age between $age1 and $age2";
...
//        }
    ...
    public function search_side(Request $request)
    {
        ...
        $_SESSION['f_status'] = request('f_status') != null ? request('f_status') : 'Any';
        $_SESSION['ad_age1'] = request('age1');
        $_SESSION['ad_age2'] = request('age2');
...
//            return view('web.candidate_list')->with(['users' => $users, 'user_count' => 0]);
    ...
    public function get_search_side(Request $request)
    {
        ...
        if (isset($_SESSION['user_master'])) {
            ...
            $sql = "SELECT * FROM `profiles` WHERE is_verified = 1 and (gender='" . $gender . "') and (" . $status . " 0) and (" . $lang . " 0) and (" . $rel . " 0) and (" . $cast . " 0) and (" . $edu . " 0) and (" . $occu . " 0) and (" . $stt . " 0) and (" . $mang . ") and (" . $phy . ") and (" . $familyin . " 0) and (" . $anual . " 0) and (" . $diet . ") and (" . $f_status . ") and age between $age1 and $age2";
...
//        }
    ...
    public function desire_candidates(Request $request)
    {
        if (isset($_SESSION['user_master'])) {
            ...
            $caste = $user->caste;
            $users = DB::table('profiles')->select(DB::raw('*'))->where(['gender' => $gender, 'religion' => $religion, 'caste' => $caste, 'is_verified' => 1])->whereBetween('age', ["$age1", "$age2"])->orderBy('id', 'desc')->paginate(5);
...
//        $caste = $_SESSION['caste'];
        ...
        $s1 = "SELECT * FROM `profiles` WHERE is_verified = 1 and age BETWEEN '$age1' and '$age2' and gender = '$gender' ORDER BY id DESC LIMIT $offset,$rowsperpage";
