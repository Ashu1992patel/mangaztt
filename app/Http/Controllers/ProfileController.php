...
class ProfileController extends Controller
{
    ...
    public function sendrequest()
    {
        if (isset($_SESSION['user_master'])) {
            $user = $_SESSION['user_master'];
            DB::table('friends')
                ->where(['user_id' => $user->id, 'friend_id' => request('search_user_id')])
                ->where(['user_id' => request('search_user_id'), 'friend_id' => $user->id])
                ->delete();
            $friend = new Friend();
    ...
    public function myprofile()
    {
        if (isset($_SESSION['user_master'])) {
            ...
            return Redirect::back()
                ->withInput()
                ->withErrors(['message' => 'Please login first']);
        }
    }
    ...
    public function admin_s_user()
    {
        ...
        if (count($users) > 0) {
            ...
            return Redirect::back()
                ->withInput()
                ->withErrors(['message' => 'No record round for search']);
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!$user_master->checkUsername(request('username'))) {
            return Redirect::back()
                ->withInput()
                ->withErrors('Username already exists in the system. Please type a different username.');
        }
        if (strlen(request('username')) < 5 || strlen(request('password')) < 5) {
            return Redirect::back()
                ->withInput()
                ->withErrors('Username or password must be at least 5 character long');
        }
